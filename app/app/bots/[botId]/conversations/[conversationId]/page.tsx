...
export default async function TranscriptPage({ params }: { params: Promise<{ botId: string; conversationId: string }> }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
      <Card title="Transcript" actions={<Link href={`/app/bots/${bot.id}/conversations`} className="text-sm text-zinc-600 hover:underline">← All conversations</Link>}>
        <ol className="space-y-3">
          {messages.map((m) => (
            <li key={m.id} className={m.role === "user" ? "flex flex-col items-end" : m.role === "system_event" ? "text-center" : ""}>
              {m.role === "system_event" ? (
                <span className="text-xs text-zinc-500">— {m.content} —</span>
              ) : (
                <>
                  ...
                  {m.tool_calls?.length ? (
                    <details className="mt-1 max-w-[85%] text-xs">
                      ...
                      <pre className="mt-1 overflow-auto rounded bg-zinc-900 p-2 text-[11px] text-zinc-100">{JSON.stringify(m.tool_calls, null, 2)}</pre>
                    </details>
                  ) : null}
                </>
              )}
      ...
      <div className="grid content-start gap-4">
        <Card title="Details">
          <dl className="grid gap-1 text-sm">
            ...
            <dt className="text-zinc-500">Page</dt>
        ...
        {lead ? (
          <Card title="Lead">
            ...
            {lead.need ? <p className="mt-1 text-sm text-zinc-700">{lead.need}</p> : null}
