...
export default async function SetupPage({ searchParams }: { searchParams: Promise<{ reason?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-10">
      ...
      {problems.length ? (
        <>
          <p className="mt-2 text-zinc-600">Fix these in <code className="rounded bg-zinc-100 px-1">.env.local</code>, then stop and restart <code className="rounded bg-zinc-100 px-1">npm run dev</code>.</p>
          <ul className="mt-5 space-y-3">
            {problems.map((p) => (
              <li key={p.name + p.problem} className="rounded-xl border border-zinc-200 bg-white p-4">
                ...
                <div className="mt-1 text-sm text-zinc-600">{p.fix}</div>
              </li>
            ))}
          </ul>
        </>
